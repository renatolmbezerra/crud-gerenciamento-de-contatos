    ...
    with st.form("new_product"):
        ...
        if submit_button:
            response = requests.post(
                "https://meu-app-backend.onrender.com/products/",
                json={
                    "name": name,
...
with st.expander("Visualizar Produtos"):
    if st.button("Exibir Todos os Produtos"):
        response = requests.get("https://meu-app-backend.onrender.com/products/")
        if response.status_code == 200:
...
with st.expander("Obter Detalhes de um Produto"):
    ...
    if st.button("Buscar Produto"):
        response = requests.get(f"https://meu-app-backend.onrender.com/products/{get_id}")
        if response.status_code == 200:
            product = response.json()
            df = pd.DataFrame([product])
...
with st.expander("Deletar Produto"):
    ...
    if st.button("Deletar Produto"):
        response = requests.delete(f"https://meu-app-backend.onrender.com/products/{delete_id}")
        show_response_message(response)
        ...
        if update_button:
            ...
            if update_data:
                response = requests.put(
                    f"https://meu-app-backend.onrender.com/products/{update_id}", json=update_data
                )
                show_response_message(response)
            else:
